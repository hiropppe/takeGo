...
def run_game_converter(cmd_line_args=None):
    ...
    parser.add_argument("--n_samples", "-n", type=int, default=None,
                        help="Number of sgf files to convert. Use for test.")
    ...
    def _walk_all_sgfs(root, n_samples):
    ...
    def _list_sgfs(path, n_samples):
        """helper function to get all SGF files in a directory (does not recurse)
        """
        files = os.listdir(path)
        return [os.path.join(path, f) for f in files if _is_sgf(f)][:n_samples]

    # get an iterator of SGF files according to command line args
    if args.directory:
        sgf_total = _count_all_sgfs(args.directory)
        if args.n_samples:
            sgf_total = min(sgf_total, args.n_samples)
        if args.recurse:
            sgf_files = _walk_all_sgfs(args.directory, sgf_total)
        else:
            sgf_files = _list_sgfs(args.directory, sgf_total)
    else:
        sgf_total = 1
